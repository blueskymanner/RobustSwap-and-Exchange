      ...
      <Options>
        <Flex mb={['8px', 0]} mr={[0, '8px']}>
          {predefinedValues.map(({ label, value: predefinedValue }) => {
            const handleClick = () => setValue(predefinedValue.toString())

            return (
              <Option key={predefinedValue}>
                <Button variant={value === predefinedValue.toString() ? 'tertiary' : 'primary'} onClick={handleClick} style={{ borderRadius: 28, color: '#5F47EE', fontWeight: 600, backgroundColor: 'transparent', border: '1px solid', width: 65, height: 32 }}>
                  {label}
                </Button>
              </Option>
            )
